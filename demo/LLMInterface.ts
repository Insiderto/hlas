...
class LLMInterface {
  ...
  private isMarkCommand(command: string): boolean {
    return (command.includes('mark') || command.includes('complete')) && 
           command.includes('todo');
  }
  ...
  private extractTodoToMark(command: string): string | null {
    ...
    const quotePattern = /mark.*['"](.+)['"].*(?:as completed|as done|complete)/i;
  ...
  private readTodos(): string {
    ...
    const todoList = screen.find(comp => 
      comp.name === 'Todo List' || 
    ...
    const todoStrings = todos.map((todo: any) => 
  ...
  private addTodo(todoText: string): string {
    ...
    const todoInput = screen.find(comp => 
    ...
    const addButton = screen.find(comp => 
  ...
  private markTodo(todoText: string): string {
    ...
    const todoItem = screen.find(comp => 
  ...
  private clearCompletedTodos(): string {
    ...
    const clearButton = screen.find(comp => 
